...
const cw = 250 * zoom;
const ch = 140 * zoom;
const rows = (q.get('ages') ?? '0.15,1').split(',').map(Number);
const ids = q.get('ids')?.split(',');
const LIST = ids ? BUILT_IN.filter((s) => ids.includes(s.id)) : BUILT_IN;
const cols = Number(q.get('cols') ?? 4);
const variant = q.get('variant');
const canvas = document.createElement('canvas');
canvas.width = cw * cols;
canvas.height = Math.ceil(LIST.length / cols) * ch * rows.length;
...
LIST.forEach((sp, i) => {
  rows.forEach((g, k) => {
    const x0 = (i % cols) * cw;
    const y0 = (Math.floor(i / cols) * rows.length + k) * ch;
    ...
    applyPose(r, pose);
    if (q.get('fly')) Object.assign(r.target, { fly: 1, flap: Number(q.get('fly')) });
    if (q.get('display')) r.target.display = Number(q.get('display'));
    r.look = { x: 90, y: 60 };
    for (let n = 0; n < 60; n++) r.update(1 / 60);
    ctx.save();
    ctx.translate(x0 + cw * 0.52, y0 + ch - 16 * zoom);
    const pal = paletteFor(sp, variant !== null ? Number(variant) : (i + k * 3) % sp.variants.length);
